...
impl From<tauri_plugin_http::reqwest::cookie::Cookie<'_>> for Cookie {
    fn from(val: tauri_plugin_http::reqwest::cookie::Cookie<'_>) -> Self {
        Cookie {
            name: val.name().to_string(),
            value: val.value().to_string(),
...
impl From<&tauri_plugin_http::reqwest::cookie::Cookie<'_>> for Cookie {
    fn from(val: &tauri_plugin_http::reqwest::cookie::Cookie) -> Self {
        Cookie {
            name: val.name().to_string(),
            value: val.value().to_string(),
            http_only: val.http_only(),
